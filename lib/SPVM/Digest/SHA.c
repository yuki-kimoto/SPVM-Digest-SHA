...
int32_t SPVM__Digest__SHA__new(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!shainit(state, alg)) {
    ...
    return env->die_v2(env, stack, "The SHA state cannot be initalized. The specified algorithm is %d.", __func__, FILE_NAME, __LINE__, alg);
  }
...
static int32_t SPVM__Digest__SHA__sha(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!obj_data) {
    return env->die_v2(env, stack, "$data must be defined.", __func__, FILE_NAME, __LINE__);
  }
...
static int32_t SPVM__Digest__SHA__hmac_sha(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (hmacinit(&hmac, alg, key, (unsigned int) len) == NULL) {
    return env->die_v2(env, stack, "HMAC cannot be initalized. The specified algorithm is %d.", __func__, FILE_NAME, __LINE__, alg);
  }
  len = env->length(env, stack, obj_data);
  while (len > MAX_WRITE_SIZE) {
